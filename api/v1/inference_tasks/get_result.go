...
func GetResult(ctx *gin.Context) {
	...
	signer := ctx.Query("signer")
	timestampStr := ctx.Query("timestamp")
	signature := ctx.Query("signature")

	if taskIdStr == "" || imageNumStr == "" || selectedNode == "" || signer == "" || timestampStr == "" || signature == "" {
	...
	match, err := ValidateSignature(signer, sigStr, timestamp, signature)
